...
COMMAND = ["/bin/bash", "-c", "python -m tools.seed_mongo_from_salesforce"]
...
for region in regions:
    ...
    platform_box_pod = None

    for pod_name, pod_namespace, pod_status in pods:
        if "platform-box-api" in pod_name and pod_status == "Running":
            platform_box_pod = (pod_name, pod_namespace)
            break
    else:
        unsuccessful_seeds.append((environment, region))

        print(f"No platform-box-api pod in environment {environment} and region {region}\n", flush=True)

        continue

    response = kubernetes_manager.execute_command(platform_box_pod[0], platform_box_pod[1], COMMAND)
